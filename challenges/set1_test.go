...
func TestChallenge2(t *testing.T) {
	...
	res := xor.XorHexStrings(buf1, buf2)
	if res != exp {
		t.Errorf("expected %s, got %s", exp, res)
	}
}
...
func TestChallenge3(t *testing.T) {
	bytes, err := encoding.HexStrToBytes("1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736")
	if err != nil {
		t.Errorf("Error converting hex string to bytes: %v", err)
	}
	m := "Cooking MC's like a pound of bacon"
	key := byte(88)
	res := decrypt.DecryptXorSingleByte(bytes, 0)
...
func TestChallenge4(t *testing.T) {
	res := decrypt.DecryptXorSingleByteFromBatchFile("../testdata/4.txt")
...
func TestChallenge5(t *testing.T) {
	xor := xor.XorRepeatingKey([]byte("Burning 'em, if you ain't quick and nimble\nI go crazy when I hear a cymbal"), []byte("ICE"))
	res := encoding.BytesToHexStr(xor)
...
func TestChallenge6(t *testing.T) {
	data := utils.ReadFile("../testdata/6.txt")
	...
	for x := range transposed {
		m := decrypt.DecryptXorSingleByte(transposed[x], x)
		key[x] = m.Key
	}
...
func TestChallenge7(t *testing.T) {
	data := utils.ReadFile("../testdata/7.txt")
	bytes, err := encoding.Base64ToBytes(data)
	if err != nil {
		t.Errorf("Error converting base64 string to bytes: %v", err)
	}
...
func TestChallenge8(t *testing.T) {
	...
	for scanner.Scan() {
		txt := scanner.Text()
		bytes, err := encoding.HexStrToBytes(txt)
		if err != nil {
			t.Errorf("Error converting hex string to bytes: %v", err)
		}
	...
	for i, line := range lines {
		lineScore := scoring.ScoringECBMode(line, 16)
		if lineScore > score {
			score = lineScore
			winner = i
		}
